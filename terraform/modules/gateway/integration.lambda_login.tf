...
resource "aws_api_gateway_resource" "proxy_lambda_login" {
...
resource "aws_api_gateway_method_response" "proxy_lambda_login" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.proxy_lambda_login.id
  http_method = aws_api_gateway_method.proxy_lambda_login.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "proxy_lambda_login" {
  ...
  resource_id = aws_api_gateway_resource.proxy_lambda_login.id
  http_method = aws_api_gateway_method.proxy_lambda_login.http_method
  status_code = aws_api_gateway_method_response.proxy_lambda_login.status_code
...
resource "aws_lambda_permission" "api_gtw_lambda_login_permission" {
